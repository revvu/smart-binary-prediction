"""
Driver script for Online Convex Optimization experiments using Numba-accelerated
implementations.

Produces figures analogous to `driver.py` but relies on `fast_algorithms` and
writes distinct outputs so results can be compared side-by-side:

- empirical_g_T_fast.png        : Empirical g(T) vs. theoretical references
- algorithm_comparison_fast.png : Algorithm comparison across sequence families
"""
...
def plot_empirical_g(T_grid: IntArray, g_emp: Mapping[int, float], *, out_path: str = "empirical_g_T_fast.png") -> None:
...
def plot_comparisons(
    ...
    *,
    out_path: str = "algorithm_comparison_fast.png",
) -> None:
...
def main() -> None:
    ...
    plot_comparisons(cfg.T_grid, stats_by_case, out_path="algorithm_comparison_fast.png")
