\section{Instance Optimal Online Learning: Achievability via SMART} \label{sec:twoapprox}

Given the setting and problem statement above, we can now present our $\bob$ policy. 
...
\begin{figure}[!t]  
  \begin{minipage}{0.32\textwidth} % Adjust the width as needed
    \centering
...
\captionsetup{format=plain}
 \vspace{-0.5em}\caption{\it\footnotesize \cref{fig:proof_intuition}(a) on the left shows the worst case instance in binary prediction for an algorithm which starts with $\ftl$ and switches at most once during the time horizon to $\cover$. \cref{fig:proof_intuition}(b) on the right depicts in a prediction with experts setting how $\bob$ resets the losses after the switch from $\ftl$ to $\wcalg$.}
  \vspace{-0.5cm}  
...
Let $\tau \defeq \min_{1 \le t \le n-1} \ftlsum_t > \theta$ denote the last round $\bob$ plays $\ftl$ before switching to $\wcalg$ (with $\tau=n$ if it never switches). $\bob$ incurs regret bounded by
\[\Reg(\bob, \ell^n) \leq \Reg(\ftl, \ell^{\tau}) + \Reg(\wcalg, \ell_{\tau+1}^n) \leq \theta + \Reg(\wcalg, \ell_{\tau+1}^n) + 1.\]
\end{lemma}
...
\begin{align*}
\Reg(\bob, \ell^n)
%&= \textstyle\sum_{t=1}^\tau \ell_t(a_t) + \textstyle\sum_{t = \tau+1}^n \ell_t(a_t) - \textstyle\sum_{t=1}^n \ell_t(a^*_n) \\
&=  \big(\textstyle\sum_{t=1}^\tau \ell_t(a_t) - \textstyle\sum_{t=1}^\tau \ell_t(a^*_n)\big) + \big(\textstyle\sum_{t=\tau+1}^n \ell_t(a_t) - \textstyle\sum_{t=\tau+1}^n \ell_t(a^*_n)\big) \\
&\stackrel{(a)}{\le} \big(\textstyle\sum_{t=1}^\tau \ell_t(a_t) - \textstyle\sum_{t=1}^\tau \ell_t(a^*_{\tau})\big) + \big(\textstyle\sum_{t=\tau+1}^n \ell_t(a_t) - \textstyle\sum_{t=\tau+1}^n \ell_t(a^*_{\tau+1:n})\big) \\
&= \Reg(\ftl, \ell^{\tau}) + \Reg(\wcalg, \ell_{\tau+1}^n).
\end{align*}
The first term is upper bounded by $\Reg(\ftl, \ell^{\tau})$ since $L_{\tau}(a^*_n) \ge L_{\tau}(a^*_{\tau})$ by definition, as $a^*_{\tau}$ is the minimizer of $L_{\tau}$, and furthermore $\bob$ always plays according to $\ftl$ in rounds up to $\tau$. The second term is upper bounded by $\Reg(\wcalg, \ell_{\tau+1}^n)$ because $\sum_{t=\tau+1}^n \ell_t(a^*_{\tau+1:n}) \leq \sum_{t=\tau+1}^n \ell_t(a^*_n)$ as $a^*_{\tau+1:n}$ is the minimizer of the losses after $\tau$, and at time $t > \tau$, $\bob$ plays according to $\wcalg$ on the sequence of losses limited to $\ell_{t+1}^n$. This is the reason that resetting the losses is an important part of the algorithm; the total regret incurred by $\bob$ can be then broken down into the sum of two independent components---the first being the regret of $\ftl$ before the switch, and the second being the regret of $\wcalg$ after. \cref{fig:proof_intuition} demonstrates in an example how the losses are reset after the switch.
Using the fact that $\ftlsum_{\tau-1} \le \theta$ and $L_{\tau-1}(x^*_{\tau-1}) \le L_{\tau-1}(x^*_{\tau})$, it follows that \\
$~~~~~~\Reg(\ftl, \ell^{\tau}) = \ftlsum_{\tau-1} + L_{\tau-1}(a^*_{\tau-1}) - L_{\tau-1}(x^*_{\tau}) + \ell_{\tau}(a^*_{\tau-1}) - \ell_{\tau}(x^*_{\tau}) \leq \theta + 1.$
\end{proof}

The reduction to ski-rental is again immediate due to the properties that $\ftlsum_t := \Reg(\ftl,\ell^{\tau})$ is adapted, monotone, and is an anytime lower bound for $\Reg(\ftl,\ell^n)$, while remaining an upper bound on the true regret incurred by $\ftl$ up to time $t$. As a result, the algorithm can pretend that it truly observes the regret it incurs at each time up to the switching time $\tau$. After the switch, $\bob$ incurs regret $\Reg(\wcalg, \ell_{\tau+1}^n)$ which is upper bounded by $g(n-\tau) \leq g(n)$ by assumption.\\
...
%             \item Let $\tau \defeq \min_{1 \le t \le n-1} \ftlsum_t >$ Threshold$_n$. Starting from $t = \tau+1$, forget $\ell^{\tau}$ and start playing $\wcalg$. 
%         \end{itemize}
%     }%
% }

{\bf Proof of Theorem~\ref{thm:2ApproxRegret}}. 
This follows immediately from Lemma \ref{lemma:regret_skirental}. For $\ell^n$ such that $\Reg(\ftl,\ell^n) \le g(n)$, $\bob$ will never switch to $\cover$ as $\ftlsum_t \le \Reg(\ftl, \ell^n) \leq g(n)$ such that $\Reg(\bob,\ell^n) = \min\{\Reg(\ftl, \ell^n), g(n)\}$. For $\ell^n$ such that $\Reg(\ftl,\ell^n) > g(n)$, by Lemma \ref{lemma:regret_skirental}, $\Reg(\bob, \ell^n)\leq g(n) + g(n-\tau) +1 \leq 2 g(n) +1$. \\% by monotonicity of $g(\cdot)$.  \begin{flushright}$\blacksquare$\end{flushright}

{\bf Proof of Theorem~\ref{thm:SkiRentalRegret}.}
...
As before, let $\tau \defeq \min_{1 \le t \le n-1} \ftlsum_t > \theta$ be the (random) round where $\bob$ switches from $\ftl$ to $\wcalg$ (with $\tau=n$ if it never switches). Then by Lemma \ref{lemma:regret_skirental} we have
\begin{align*}
...
% \begin{proof}
%     The proof is almost identical to that of~\cref{thm:2ApproxRegret}, with the only change being that when $g(L^*) \ge \Reg(\ftl,\ell^n)$, then (recalling that $\tau$ denotes the switch point) 
%     \[
%     \Reg(\bob,\ell^n) \le g(L^*)+1+g\left(\min_{j}\sum_{t=\tau+1}^n \ell_{tj}\right) \le 2g(L^*) + 1
%     \]
%     since $\min_{j}\sum_{t=\tau+1}^n \ell_{tj} \le \min_{j}\sum_{t=1}^n \ell_{tj} \le L^*$ and $g(\cdot)$ is monotone.
% \end{proof}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
